...
def chatbot(prompt):
    ...
    functions = [
        ...
        # {
        #     "name":"get_books",
        #     "description": "Retreives the names of all books in the library inventory.",
        #     "parameters": {
        #         "type" : "object",
        #         "properties" : {}
        #     }
    ...
    try:
        ...
        if chat_message.function_call:
            ...
            if func_name == "list_books":
                result = list_books(sheet)
            elif func_name == "search_books":
                result = search_books(sheet, **func_args)
            elif func_name == "add_book":
                result = add_book(sheet, **func_args)
            elif func_name == "update_book":
                result = update_book(sheet, **func_args)
            elif func_name == "remove_book":
                result = remove_book(sheet, **func_args)
            elif func_name == "check_availability":
                result = check_availability(sheet, **func_args)
            elif func_name == "count_books":
                result = count_books(sheet)
            elif func_name == "get_available_books":
                result = get_available_books(sheet)
            elif func_name == "get_issued_books" :
                result = get_issued_books(sheet)
            ...
            # Print the response if no function is called
            print(f"Bot: {chat_message.content}")
